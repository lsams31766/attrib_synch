...
from netauth_req import get_creds
...
def delete_user(login_creds, delete_user_dn): 
    ...
    try:
        print(f"try to delete---{delete_user_dn}---")
...
def get_creds_from_server(dirname):
    ...
    creds = get_creds(dirname)
    fixed = '|'.join([creds['fqdn'], creds['pass'], creds['authdn'], creds['port']])
    #creds = get_login_info(out_list)
    #return creds  # return in old nc.pl format for compatibility
    '''
    result = subprocess.run(['perl',ABS_NC_FN, dirname], stdout=subprocess.PIPE)
    l = len(result.stdout)
    print(l)
...
if __name__ == '__main__':
    ...
    enterprise_creds = get_creds_from_server('dirtest-cl')
    search_base_enterprise  = 'o=bms.com'
    search_filter = '(bmsid=00004750)'
    attrs = ['cn','sn','uid','bmsentaccountstatus','extensionAttribute15'] 
    s,results = ldap_search(enterprise_creds, search_base_enterprise, search_filter, 
        search_scope, attrs)
