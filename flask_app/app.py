...
def process_query(servers_selected, new_attributes, new_filter,env_selected):
    ...
    for s in servers_selected:
        ...
        if d['creds'] == 'local': # use creds.json file
            ...
            # fix for now ad one wrong addresses
            if dir_creds[0] == 'adjoin-na.one.ads.bms.com':
                dir_creds[1] = 'CrasuT5Uzaq?XEt'
                dir_creds[2] = 'CN=APP_JOINENGINE,OU=Service Accounts,OU=IMSS,DC=one,DC=ads,DC=bms,DC=com'
                dir_creds[3] = '636'
            # fix for fruat which is uat fru
            if dir_creds[0] == 'frds1.aws.bms.com' and (env_selected == 'UAT'):
                dir_creds[0] = 'fruat1.aws.bms.com'
...
@app.route("/attribsynch/query", methods=['POST'])
def query():
    print('GOT QUERY!')
